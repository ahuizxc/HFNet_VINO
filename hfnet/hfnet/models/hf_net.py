...
def local_head(features, config):
    ...
    with tf.variable_scope('detector', reuse=tf.AUTO_REUSE):
        ...
        prob = tf.depth_to_space(prob, config['detector_grid'], data_format="NHWC")
        # pdb.set_trace()
...
class HfNet(BaseModel):
    ...
    def _model(self, inputs, mode, **config):
        ...
        if mode == Mode.PRED:
            ...
            # return {"scores_dense": tf.identity(ret['scores_dense'], name="scores_dense")}
            if config['local']['nms_radius']:
                scores = simple_nms(scores, config['local']['nms_radius'])

            # with tf.name_scope('keypoint_extraction'):

                # scores_reshaped = tf.reshape(scores[0],[1,-1])
                # keypoints = tf.argsort(scores_reshaped, direction='DESCENDING')
                # keypoints_row = tf.cast(keypoints/224,tf.int32)
                # keypoints_column = keypoints%224
                # keypoints_final = tf.concat([keypoints_row, keypoints_column], axis=0)
                # scores_final = tf.sort(scores_reshaped, direction='DESCENDING')
            ...
            # pdb.set_trace()
            # scores_final = scores_final[None]
            ...
            with tf.name_scope('descriptor_sampling'):
                ...
                # pdb.set_trace()
                # local_descriptors = \
                #         tf.contrib.resampler.resampler(desc, tf.to_float(scaling)[::-1]*tf.to_float(keypoints))
                # local_descriptors_resampled = tf.reshape(tf.image.resize_bilinear(desc, size=tf.shape(image)[1:3]),(1,-1,256))
                # local_descriptors = tf.gather(local_descriptors_resampled[0], keypoints[0])
